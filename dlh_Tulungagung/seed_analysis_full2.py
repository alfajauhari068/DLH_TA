...
def parse_migration(path):
    ...
    for stmt in re.finditer(r"\$table->([A-Za-z0-9_]+)\(([^)]*)\)([^;]*);", text, re.S):
        ...
        if method == 'unique' or '->unique' in chain:
            uniques.append(col)
        if 'foreign' in method or method == 'foreignId' or 'constrained' in chain:
            target = None
            ...
            on_delete = None
            on_update = None
            m2 = re.search(r"->onDelete\(\s*['\"]([^'\"]+)['\"]\s*\)", chain)
            if m2:
                on_delete = m2.group(1)
            m2 = re.search(r"->onUpdate\(\s*['\"]([^'\"]+)['\"]\s*\)", chain)
            if m2:
    ...
    return {'file': path.name, 'table': table, 'mode': mode, 'columns': columns, 'foreign_keys': fks, 'uniques': uniques, 'enums': enums, 'text': text}


def parse_model(path):
    text = read_text(path)
    # class name definition regardless of extends
    cls = None
    if m := re.search(r'class\s+([A-Za-z0-9_]+)\b', text):
        cls = m.group(1)
    namespace = None
    ...
    return {'file': path.name, 'class': cls, 'namespace': namespace, 'uses': uses, 'props': props, 'timestamps': ts, 'relations': relations, 'text': text}
...
def parse_seeder(path):
    ...
    # include simple unqualified references in DB queries? ignore for now
    calls = []
    for block in re.findall(r'\$this->call\(\[([^\]]+)\]\)', text, re.S):
    ...
    return {'file': path.name, 'class': cls, 'namespace': namespace, 'uses': uses, 'model_refs': models, 'calls': calls, 'db_tables': inserts, 'factory_usage': factories, 'text': text}
...
def parse_factory(path):
    ...
    if m := re.search(r'namespace\s+([^;]+);', text):
        namespace = m.group(1).strip()
    uses = re.findall(r'use\s+([^;]+);', text)
    return {'file': path.name, 'class': cls, 'model': model, 'namespace': namespace, 'uses': uses, 'text': text}
...
with open(root / 'seed_analysis_full2.json', 'w', encoding='utf-8') as f:
