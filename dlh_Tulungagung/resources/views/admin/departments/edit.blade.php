...
@section('subtitle', 'Edit department: ' . $department->name)
...
@section('content')
    <form action="{{ route('admin.departments.update', $department) }}" method="POST" class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        @csrf
        @method('PUT')
        ...
        <div class="lg:col-span-8 space-y-6">
            <x-admin.form.card title="Department Information" padding="p-6">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $department->name) }}" class="w-full border border-gray-200 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all @error('name') border-red-500 @enderror" required>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4" class="w-full border border-gray-200 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all @error('description') border-red-500 @enderror">{{ old('description', $department->description) }}</textarea>
